...
public class RedisNode implements KVNode {

    private static final String REDIS_PROCESS_NAME = "redis-server";
    String node;
    List<String> nodes;
    private static final Logger log = LoggerFactory.getLogger(RedisNode.class);
    private String installDir = "redis-chaos-test";
    private String redisVersion = "5.0.5";


    public RedisNode(String node, List<String> nodes, RedisConfig redisConfig) {
        this.node = node;
        this.nodes = nodes;
    ...
    public void start() {
        try {
            log.info("Node {} start redis server...", node);
            SshUtil.execCommandInDir(node, installDir, "redis-server redis.conf");

        } catch (Exception e) {
            log.error("Node {} start redis node failed", node, e);
            throw new RuntimeException(e);
        }
    }
    ...
    public void stop() {
        try {
            KillProcessUtil.kill(node, REDIS_PROCESS_NAME);
        } catch (Exception e) {
    ...
    public void kill() {
        try {
            KillProcessUtil.forceKill(node, REDIS_PROCESS_NAME);
        } catch (Exception e) {
    ...
    public void pause() {
        try {
            PauseProcessUtil.suspend(node, REDIS_PROCESS_NAME);
        } catch (Exception e) {
    ...
    public void resume() {
        try {
            PauseProcessUtil.resume(node, REDIS_PROCESS_NAME);
        } catch (Exception e) {

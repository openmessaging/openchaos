...
public class NetUtil {
    ...
    private static final String delayMean = "100ms";
    ...
    public static void delay(String node) throws Exception {
        SshUtil.execCommandWithArgs(node, "tc", "qdisc", "add", "dev", "eth0", "root", "netem", "delay", delayMean, delayVariance);
    }

    public static void healDelay(String node) throws Exception {
        String output = null;
        try {
            output = SshUtil.execCommandWithArgsReturnStr(node, "tc", "qdisc", "del", "dev", "eth0", "root", "netem", "delay", delayMean, delayVariance);
        } catch (RemoteException e) {
    ...
    public static void loss(String node, List<String> targetNodes) throws Exception {
        for (String targetNode : targetNodes) {
            ...
            SshUtil.execCommandWithArgsReturnStr(node, "iptables", "-A", "INPUT", "-s", targetNode, "-m", "statistic", "--mode", "random", "--probability", lossRate, "-j", "DROP");
        }
    }
